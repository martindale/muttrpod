...
Identity.methods.getConversations = function(callback) {
  ...
  Message.find(query).sort(desc).exec(function(err, messages) {
    ...
    callback(null, self._sortMessages(messages));
  });
};
...
*/
Identity.methods._sortMessages = function(messages) {
  var conversations = {};

  for (var m = 0; m < messages.length; m++) {
    var msg = messages[m];

    if (!conversations[msg.sender.userID]) {
      conversations[msg.sender.userID] = [];
    }

    conversations[msg.sender.userID].push(msg);
  }
...
* @param {function} callback
*/
Identity.methods.purgeConversations = function(callback) {
