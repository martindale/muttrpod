...
Router.prototype._getConversations = function(req, res, callback) {
  ...
  identity.getConversations(function(err, conversations) {
    if (err) {
      return callback(err);
    }

    self._success(res, { conversations: conversations });
  });
};
...
Router.prototype._purgeConversations = function(req, res, callback) {
  ...
  identity.purgeConversations(function(err) {
    if (err) {
      return callback(err);
    }

    self._getConversations(req, res, callback);
  });
};
