...
Router.prototype._getInfo = function(req, res, callback) {
  ...
  DHTEntry.count({}, function(err, count) {
...
Router.prototype._storeMessageInDHT = function(req, res, callback) {
  var self = this;
  var rawtext = req.rawbody; // should be Hex(ASCIIArmoredPGPMessage)
  var key = new sha3.SHA3Hash.update(rawtext).digest('hex');

  this._dht.put(key, rawtext, function(err) {
    if (err) {
      return callback(err);
    }
...
Router.prototype._getRemoteIdentity = function(alias, host, callback) {
  ...
  request.get({ url: url }, function(err, res, pubkey) {
    ...
    var pubkeyhash = new sha3.SHA3Hash(512).update(pubkey).digest('hex');
