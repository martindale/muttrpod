...
*/
Bus.prototype._handleMessage = function(sock, message) {
  var self = this;
  ...
  try {
    pgpMessage = pgp.message.readArmored(message);
    parsedMsg = qs.parse(pgpMessage.getText());

    assert(parsedMsg.identity, 'No identity supplied');
    assert(parsedMsg.identity_type === 'pubkeyhash', 'Type is not pubkeyhash');
