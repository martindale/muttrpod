...
Server.prototype.start = function(onReady) {
  ...
  async.series(initializers, function(err) {
    ...
    self._log.info('joined network on port: %s', self.config.network.port);
    self._log.info('http(s) and ws(s) server: %s', self.config.server.port);
...
Server.prototype._configureApp = function(callback) {
  ...
  this._app.use(authenticate);
  this._app.use(authorize);

  this._router.mount(this._app);
...
Server.prototype._startServer = function(callback) {
  ...
  if (useSSL) {
    this._server = https.createServer({
      ca: this.config.server.ca.map(fs.readFileSync(ca)),
  ...
  this._rpc = new RPC(this._wsserver);
...
Server.prototype._startRedirector = function(callback) {
  ...
  this._redirector.listen(port, address, callback);
};
