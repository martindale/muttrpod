...
Server.prototype._startServer = function(callback) {
  ...
  var pipeline = {};
  if (useSSL) {
    pipeline.keys = function createCertificate( done ) {
      pem.createCertificate({ days: 1, selfSigned: true }, done );
    };
  }
  ...
  async.series(pipeline, function(err, results) {
    ...
    if (useSSL) {
      self._server = https.createServer({
        key: results.keys.serviceKey,
        cert: results.keys.certificate,
        requestCert: false,
    ...
    self._wsserver = new WSServer({ server: self._server });
    self._bus = new Bus({ wsserver: self._wsserver, logger: self._log });
    self._router = Router().mount(self);

    self._server.listen(port, address, function(err) {
      if (err) {
        return callback(err);
      }
